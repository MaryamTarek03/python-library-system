...
class Home(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # font
        font_main = ctk.CTkFont(family="Gabriola", size = 18, weight = 'bold')
        ...
        # :: search bar 
        self.search_bar = w.search_bar(self.search_area, hint = 'Search for books', font = font_main)
        self.search_bar.grid(row = 0 , column = 0, sticky = 'new', padx = (c.padding, 5), pady = c.padding)

        # :: search button
        self.search_button = w.search_button(self.search_area, text = 'search', font = font_main)
...
class SummaryItem(ctk.CTkFrame):
    def __init__(self, parent, text, number):
        ...
        # attributes
        self.number = ctk.CTkLabel(self, text = f'{number}', font = ctk.CTkFont('Papyrus', size = 50, weight = 'bold'))
        self.text = ctk.CTkLabel(self, text = text, font = ctk.CTkFont('Papyrus', size = 20))
